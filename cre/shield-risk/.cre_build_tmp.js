...
var RELOAD_WINDOW_SECS = 3 * 3600;
var H = 3600;
var D = 24 * H;
var isTopUp = (f) => f.kind === "TOP_UP_INSTANT" || f.kind === "TOP_UP_GATED";
...
function windowStats(flows, sessions, now, hours) {
  const from2 = now - hours * H;
  const inWindow = flows.filter((f) => f.blockTime >= from2);
  let sent = 0n, returned = 0n, topUpCount = 0;
  ...
  let realisedLoss = 0n, realisedGain = 0n, lossSessions = 0;
  for (const s of sessions) {
    if (!s.realised || s.lastActivityAt < from2)
      continue;
    if (s.net < 0n) {
      realisedLoss += -s.net;
      lossSessions += 1;
...
function deriveProfile(vault, flowsIn, now) {
  ...
  for (const f of flows) {
    if (isTopUp(f) || f.kind === "RETURN") {
      ...
      totals.deposited += f.amount;
    else
      totals.exited += f.amount;
  }
  ...
  const velocity24h = flows.filter((f) => f.blockTime >= now - D && (isTopUp(f) || f.kind === "COLD_TRANSFER")).reduce((a, f) => a + f.amount, 0n);
  ...
  const reloadsAfterLoss7d = flows.filter((f) => isTopUp(f) && lossTimes.some((t) => f.blockTime > t && f.blockTime - t <= RELOAD_WINDOW_SECS)).length;
  return {
    vault,
    asOf: now,
    totals,
...
function abytes3(value2, length, title = "") {
  if (isBytes4(value2) && (length === undefined || value2.length === length))
    return value2;
  if (length !== undefined)
...
class HashMD2 {
  ...
  update(data) {
    ...
    for (let pos = 0;pos < len2; ) {
      ...
      buffer.set(pos === 0 && take === len2 ? data : data.subarray(pos, pos + take), this.pos);
      this.pos += take;
      pos += take;
      if (this.pos === blockLen) {
        this.process(view, 0);
...
class SHA2_32B extends HashMD2 {
  ...
  get() {
    const { A, B, C, D: D2, E: E2, F, G, H: H2 } = this;
  ...
  process(view, offset) {
    ...
    for (let i2 = 0;i2 < 64; i2++) {
      ...
      const T1 = H2 + sigma1 + Chi2(E2, F, G) + SHA256_K2[i2] + SHA256_W2[i2] | 0;
...
var sha2563 = /* @__PURE__ */ createHasher3(() => new _SHA256, /* @__PURE__ */ oidNist(1));
var sha512 = /* @__PURE__ */ createHasher3(() => new _SHA512, /* @__PURE__ */ oidNist(3));
/*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) */
...
var abytes4 = (value2, length, title) => abytes3(value2, length, title);
var anumber3 = anumber2;
function astring(value2, title = "") {
  if (typeof value2 !== "string") {
    const prefix = title && `"${title}" `;
    throw new TypeError(prefix + "expected string, got type=" + typeof value2);
  }
  return value2;
}
...
function afunction(value2, title) {
  if (typeof value2 !== "function")
    throw new TypeError(`"${title}" is invalid: expected function, got ${typeof value2}`);
  return value2;
}
var bytesToHex4 = bytesToHex3;
var concatBytes6 = (...arrays) => concatBytes5(...arrays);
var hexToBytes5 = (hex) => hexToBytes4(hex);
var isBytes5 = isBytes4;
var randomBytes3 = (bytesLength) => randomBytes2(bytesLength);
var _0n7 = /* @__PURE__ */ BigInt(0);
var _1n7 = /* @__PURE__ */ BigInt(1);
var atitle2 = (title) => title ? `"${title}" ` : "";
function abool3(value2, title = "") {
  if (typeof value2 !== "boolean")
    throw new TypeError(atitle2(title) + "expected boolean, got type=" + typeof value2);
  return value2;
}
...
function asafenumber(value2, title = "") {
  if (typeof value2 !== "number") {
    const prefix = title && `"${title}" `;
    throw new TypeError(prefix + "expected number, got type=" + typeof value2);
  }
  if (!Number.isSafeInteger(value2)) {
    const prefix = title && `"${title}" `;
    throw new RangeError(prefix + "expected safe integer, got " + value2);
  }
}
...
function bytesToNumberLE2(bytes) {
  return hexToNumber4(bytesToHex3(copyBytes(abytes3(bytes)).reverse()));
}
...
function createHmacDrbg2(hashLen, qByteLen, hmacFn) {
  ...
  const h = (...msgs) => hmacFn(k, concatBytes6(v, ...msgs));
...
function validateObject2(object, fields2 = {}, optFields = {}, title = "object") {
...
function invertCt(a, prime) {
  ...
  const inverse = pow(an, prime - _2n5, prime);
...
function sqrt5mod82(Fp, n) {
  ...
  const nv = F.mul(n, v);
  const i2 = F.mul(F.mul(nv, _2n5), v);
...
class _Field {
  ...
  isValid(num2) {
    ...
    return _0n8 <= num2 && num2 < this.ORDER;
  }
  is0(num2) {
    return num2 === _0n8;
  }
...
class ScalarMultiplier {
  ...
  mulCTBlinded(point, scalar, transform) {
    ...
    if (!isBytes5(blind) || blind.length !== BLIND_BYTES)
...
function edwards(params, extraOpts = {}) {
  ...
  const randomBytes4 = opts.randomBytes === undefined ? randomBytes3 : opts.randomBytes;
  const MASK = _2n6 << BigInt(Fp.BYTES * 8) - _1n10;
  function isOdd(n) {
    if (!Fp.isOdd)
  ...
  class Point2 {
    ...
    static fromBytes(bytes, zip215 = false) {
      const len2 = Fp.BYTES;
      const { a, d } = CURVE;
      bytes = copyBytes(abytes4(bytes, len2, "point"));
    ...
    static fromHex(hex, zip215 = false) {
      return Point2.fromBytes(hexToBytes5(hex), zip215);
    }
    get x() {
      return this.toAffine().x;
    }
    ...
    double() {
      ...
      const E2 = Fp.sub(Fp.subN(Fp.sqr(x1y1), A), B);
      const G = Fp.addN(D2, B);
      const F = Fp.subN(G, C);
      const H2 = Fp.subN(D2, B);
      const X3 = Fp.mul(E2, F);
      const Y3 = Fp.mul(G, H2);
      const T3 = Fp.mul(E2, H2);
      const Z3 = Fp.mul(F, G);
    ...
    add(other) {
      ...
      const E2 = Fp.sub(Fp.subN(Fp.mulN(Fp.addN(X1, Y1), Fp.addN(X2, Y2)), A), B);
      const F = Fp.subN(D2, C);
      const G = Fp.addN(D2, C);
      const H2 = Fp.sub(B, mulA(A));
      const X3 = Fp.mul(E2, F);
      const Y3 = Fp.mul(G, H2);
      const T3 = Fp.mul(E2, H2);
      const Z3 = Fp.mul(F, G);
...
function eddsa(Point2, cHash, eddsaOpts = {}) {
  ...
  function getPrivateScalar(key) {
    ...
    const hashed = abytes4(hash(key), 2 * len2, "hashedSecretKey");
  ...
  function hashDomainToScalar(context = Uint8Array.of(), ...msgs) {
    ...
    return modN_LE(hash(domain(msg, abytes4(context, undefined, "context"), !!prehash)));
  }
  function sign(msg, secretKey, options = {}) {
    validateObject2(options, {}, {}, "options");
    msg = copyBytes(abytes4(msg, undefined, "message"));
    if (prehash)
      msg = prehash(msg);
    ...
    const rs = concatBytes6(R, Fn.toBytes(s));
  ...
  function verify(sig, msg, publicKey, options = verifyOpts) {
    ...
    sig = abytes4(sig, len2, "signature");
    msg = abytes4(msg, undefined, "message");
    ...
    const s = bytesToNumberLE2(sig.subarray(mid, len2));
    let A, R, SB;
    try {
  ...
  function randomSecretKey(seed) {
    seed = seed === undefined ? randomBytes4(lengths.seed) : seed;
  ...
  function isValidPublicKey(key, zip215) {
    try {
      return !!Point2.fromBytes(key, zip215 === undefined ? verifyOpts.zip215 : zip215);
    } catch (error2) {
      return false;
    }
  }
  const utils2 = {
    ...
    toMontgomery(publicKey) {
      ...
      return toMontgomery(Point2.fromBytes(publicKey));
    },
...
function toMontgomerySecret(secretKey) {
  const size3 = ed25519_Point.Fp.BYTES;
  abytes3(secretKey, size3);
  return adjustScalarBytes(sha512(secretKey.subarray(0, size3))).subarray(0, size3);
}
function ed(opts) {
...
var EncodeBuffer = function() {
  ...
  EncodeBuffer2.prototype.store_value = function(value2, type) {
    var bSize = type.substring(1);
    var size3 = parseInt(bSize) / 8;
    this.resize_if_necessary(size3);
    var toCall = type[0] === "f" ? "setFloat".concat(bSize) : type[0] === "i" ? "setInt".concat(bSize) : "setUint".concat(bSize);
    this.view[toCall](this.offset, value2, true);
    this.offset += size3;
  };
...
var DecodeBuffer = function() {
  ...
  DecodeBuffer2.prototype.assert_enough_buffer = function(size3) {
    if (this.offset + size3 > this.buffer.byteLength) {
  ...
  DecodeBuffer2.prototype.consume_value = function(type) {
    var bSize = type.substring(1);
    var size3 = parseInt(bSize) / 8;
    this.assert_enough_buffer(size3);
  ...
  DecodeBuffer2.prototype.consume_bytes = function(size3) {
    this.assert_enough_buffer(size3);
    var ret = this.buffer.slice(this.offset, this.offset + size3);
    this.offset += size3;
    return ret;
  };
...
function isArrayLike(value2) {
  return Array.isArray(value2) || !!value2 && typeof value2 === "object" && "length" in value2 && typeof value2.length === "number" && (value2.length === 0 || value2.length > 0 && (value2.length - 1 in value2));
}
function expect_type(value2, type, fieldPath) {
  if (typeof value2 !== type) {
    throw new Error("Expected ".concat(type, " not ").concat(typeof value2, "(").concat(value2, ") at ").concat(fieldPath.join(".")));
  }
}
function expect_bigint(value2, fieldPath) {
  var basicType = ["number", "string", "bigint", "boolean"].includes(typeof value2);
  var strObject = typeof value2 === "object" && value2 !== null && "toString" in value2;
  if (!basicType && !strObject) {
    throw new Error("Expected bigint, number, boolean or string not ".concat(typeof value2, "(").concat(value2, ") at ").concat(fieldPath.join(".")));
  }
}
...
function expect_enum(value2, fieldPath) {
  if (typeof value2 !== "object" || value2 === null) {
    throw new Error("Expected object not ".concat(typeof value2, "(").concat(value2, ") at ").concat(fieldPath.join(".")));
  }
}
...
var BorshSerializer = function() {
  ...
  BorshSerializer2.prototype.encode = function(value2, schema) {
    this.encode_value(value2, schema);
    return this.encoded.get_used_buffer();
  };
  BorshSerializer2.prototype.encode_value = function(value2, schema) {
    if (typeof schema === "string") {
      if (integers.includes(schema))
        return this.encode_integer(value2, schema);
      if (schema === "string")
        return this.encode_string(value2);
      if (schema === "bool")
        return this.encode_boolean(value2);
    }
    if (typeof schema === "object") {
      if ("option" in schema)
        return this.encode_option(value2, schema);
      if ("enum" in schema)
        return this.encode_enum(value2, schema);
      if ("array" in schema)
        return this.encode_array(value2, schema);
      if ("set" in schema)
        return this.encode_set(value2, schema);
      if ("map" in schema)
        return this.encode_map(value2, schema);
      if ("struct" in schema)
        return this.encode_struct(value2, schema);
    }
  };
  BorshSerializer2.prototype.encode_integer = function(value2, schema) {
    var size3 = parseInt(schema.substring(1));
    if (size3 <= 32 || schema == "f64") {
      this.checkTypes && expect_type(value2, "number", this.fieldPath);
      this.encoded.store_value(value2, schema);
    } else {
      this.checkTypes && expect_bigint(value2, this.fieldPath);
      this.encode_bigint(BigInt(value2), size3);
    }
  };
  BorshSerializer2.prototype.encode_bigint = function(value2, size3) {
    var buffer_len = size3 / 8;
    var buffer = new Uint8Array(buffer_len);
    for (var i2 = 0;i2 < buffer_len; i2++) {
      buffer[i2] = Number(value2 & BigInt(255));
      value2 = value2 >> BigInt(8);
    }
    this.encoded.store_bytes(new Uint8Array(buffer));
  };
  BorshSerializer2.prototype.encode_string = function(value2) {
    this.checkTypes && expect_type(value2, "string", this.fieldPath);
    var _value = value2;
    var utf8Bytes = [];
  ...
  BorshSerializer2.prototype.encode_boolean = function(value2) {
    this.checkTypes && expect_type(value2, "boolean", this.fieldPath);
    this.encoded.store_value(value2 ? 1 : 0, "u8");
  };
  BorshSerializer2.prototype.encode_option = function(value2, schema) {
    if (value2 === null || value2 === undefined) {
      this.encoded.store_value(0, "u8");
    } else {
      this.encoded.store_value(1, "u8");
      this.encode_value(value2, schema.option);
    }
  };
  BorshSerializer2.prototype.encode_enum = function(value2, schema) {
    this.checkTypes && expect_enum(value2, this.fieldPath);
    var valueKey = Object.keys(value2)[0];
    for (var i2 = 0;i2 < schema["enum"].length; i2++) {
      var valueSchema = schema["enum"][i2];
      if (valueKey === Object.keys(valueSchema.struct)[0]) {
        this.encoded.store_value(i2, "u8");
        return this.encode_struct(value2, valueSchema);
      }
    }
    throw new Error("Enum key (".concat(valueKey, ") not found in enum schema: ").concat(JSON.stringify(schema), " at ").concat(this.fieldPath.join(".")));
  };
  BorshSerializer2.prototype.encode_array = function(value2, schema) {
    if (isArrayLike(value2))
      return this.encode_arraylike(value2, schema);
    if (value2 instanceof ArrayBuffer)
      return this.encode_buffer(value2, schema);
    throw new Error("Expected Array-like not ".concat(typeof value2, "(").concat(value2, ") at ").concat(this.fieldPath.join(".")));
  };
  BorshSerializer2.prototype.encode_arraylike = function(value2, schema) {
    if (schema.array.len) {
      expect_same_size(value2.length, schema.array.len, this.fieldPath);
    } else {
      this.encoded.store_value(value2.length, "u32");
    }
    for (var i2 = 0;i2 < value2.length; i2++) {
      this.encode_value(value2[i2], schema.array.type);
    }
  };
  BorshSerializer2.prototype.encode_buffer = function(value2, schema) {
    if (schema.array.len) {
      expect_same_size(value2.byteLength, schema.array.len, this.fieldPath);
    } else {
      this.encoded.store_value(value2.byteLength, "u32");
    }
    this.encoded.store_bytes(new Uint8Array(value2));
  };
  BorshSerializer2.prototype.encode_set = function(value2, schema) {
    this.checkTypes && expect_type(value2, "object", this.fieldPath);
    var isSet = value2 instanceof Set;
    var values = isSet ? Array.from(value2.values()) : Object.values(value2);
  ...
  BorshSerializer2.prototype.encode_map = function(value2, schema) {
    this.checkTypes && expect_type(value2, "object", this.fieldPath);
    var isMap = value2 instanceof Map;
    var keys = isMap ? Array.from(value2.keys()) : Object.keys(value2);
    this.encoded.store_value(keys.length, "u32");
    for (var _i = 0, keys_1 = keys;_i < keys_1.length; _i++) {
      var key = keys_1[_i];
      this.encode_value(key, schema.map.key);
      this.encode_value(isMap ? value2.get(key) : value2[key], schema.map.value);
    }
  };
  BorshSerializer2.prototype.encode_struct = function(value2, schema) {
    this.checkTypes && expect_type(value2, "object", this.fieldPath);
    for (var _i = 0, _a = Object.keys(schema.struct);_i < _a.length; _i++) {
      var key = _a[_i];
      this.fieldPath.push(key);
      this.encode_value(value2[key], schema.struct[key]);
      this.fieldPath.pop();
    }
  };
...
var BorshDeserializer = function() {
  ...
  BorshDeserializer2.prototype.decode_integer = function(schema) {
    ...
    return this.decode_bigint(size3, schema.startsWith("i"));
  };
  BorshDeserializer2.prototype.decode_bigint = function(size3, signed) {
    if (signed === undefined) {
      signed = false;
    }
    var buffer_len = size3 / 8;
    ...
    if (signed && buffer[buffer_len - 1]) {
      return BigInt.asIntN(size3, BigInt("0x".concat(bits)));
    }
    return BigInt("0x".concat(bits));
  };
  BorshDeserializer2.prototype.decode_string = function() {
    var len2 = this.decode_integer("u32");
    var buffer = new Uint8Array(this.buffer.consume_bytes(len2));
    var codePoints = [];
    for (var i2 = 0;i2 < len2; ++i2) {
      ...
      if (byte < 128) {
        ...
        codePoints.push((byte & 31) << 6 | buffer[++i2] & 63);
      } else if (byte < 240) {
        codePoints.push((byte & 15) << 12 | (buffer[++i2] & 63) << 6 | buffer[++i2] & 63);
      } else {
        var codePoint = (byte & 7) << 18 | (buffer[++i2] & 63) << 12 | (buffer[++i2] & 63) << 6 | buffer[++i2] & 63;
  ...
  BorshDeserializer2.prototype.decode_array = function(schema) {
    var result = [];
    var len2 = schema.array.len ? schema.array.len : this.decode_integer("u32");
    for (var i2 = 0;i2 < len2; ++i2) {
  ...
  BorshDeserializer2.prototype.decode_map = function(schema) {
    ...
    for (var i2 = 0;i2 < len2; ++i2) {
      ...
      var value2 = this.decode_value(schema.map.value);
...
function serialize(schema, value2, validate) {
  ...
  return serializer.encode(value2, schema);
}
var VERDICT_TTL_SECS = 15 * 60;
var RISK_VERDICT_SCHEMA = {
  struct: {
...
function encodeVerdictMessage(v) {
  return serialize(RISK_VERDICT_SCHEMA, { ...v, signature: new Uint8Array(64) });
}
function verdictToJson(v, verifier) {
  return {
    ...
    evidenceHash: toHex2(v.evidenceHash),
    signature: toHex2(v.signature),
...
function canonicalJson(value2) {
  ...
  return JSON.stringify(norm(value2));
}
function evidenceHashOf(bundle) {
  return sha2563(new TextEncoder().encode(canonicalJson(bundle)));
}
...
function assess(profile, policy, now, sinceLossAt = 0, venue) {
  ...
  let reasonCode = REASON.REALIZED_LOSS;
  if (profile.reloadsAfterLoss7d > 0)
    reasonCode = REASON.RELOAD_AFTER_LOSS;
  else if (profile.lossStreak >= 2)
    reasonCode = REASON.LOSS_STREAK;
  const lines = [];
  ...
  for (const s of sessionsInWindow.slice(-3)) {
    ...
    lines.push(s.net < 0n ? `Sent ${usd(s.sent)}, ${usd(s.returned)} came back: ${usd(-s.net)} lost, ${ago}.` : `Sent ${usd(s.sent)}, ${usd(s.returned)} came back: ${usd(s.net)} gained, ${ago}.`);
  }
  if (venueLoss > 0n)
    lines.push(`${usd(venueLoss)} realised on Hyperliquid ${venue.network} over ${venue.fills} fill${venue.fills === 1 ? "" : "s"}, read from the venue's own API.`);
  if (profile.lossStreak >= 2)
    lines.push(`${profile.lossStreak} losing sessions in a row.`);
  if (profile.reloadsAfterLoss7d > 0)
    lines.push(`Reloaded within 3 hours of a loss ${profile.reloadsAfterLoss7d} time${profile.reloadsAfterLoss7d === 1 ? "" : "s"} this week.`);
  const headline = triggered ? `You realised ${usd(realizedLossUsdc)} in losses in the last 24 hours. Your rule pauses new capital for ${hours(policy.lossCooldownSecs)}.` : `Realised losses in the last 24 hours: ${usd(realizedLossUsdc)}, below your ${usd(policy.lossTriggerUsdc)} trigger.`;
  const evidence = {
    version: 1,
    vault: profile.vault,
    asOf: now,
    ...
    ...venueDominates ? { venue: { source: "hyperliquid", network: venue.network, account: venue.account, realisedLossUsdc: venueLoss.toString(), fills: venue.fills, lastFillAt: venue.lastFillAt } } : {}
  };
  return {
    triggered,
    actionable,
...
function buildUnsignedVerdict(a, policy, nonce, now) {
  return {
...
function signVerdict(unsigned, secretKey) {
...
for (let round = 0, R = _1n12, x = 1, y = 0;round < 24; round++) {
  ...
  let t = _0n11;
  for (let j = 0;j < 7; j++) {
    R = (R << _1n12 ^ (R >> _7n3) * _0x71n2) % _256n2;
    if (R & _2n8)
...
var genKeccak = (suffix, blockLen, outputLen, info = {}) => createHasher3(() => new Keccak2(blockLen, suffix, outputLen), info);
...
var _DER = {
  ...
  _tlv: {
    ...
    decode(tag, data) {
      const { Err: E2 } = _DER;
      data = abytes4(data, undefined, "DER data");
      let pos = 0;
      if (tag < 0 || tag > 255)
  ...
  toSig(bytes, maxScalarBytes) {
    ...
    const data = abytes4(bytes, undefined, "signature");
    const { v: seqBytes, l: seqLeftBytes } = tlv.decode(48, data);
    if (seqLeftBytes.length)
...
/*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) */
var divNearest2 = (num2, den) => (num2 + (num2 >= 0 ? den : -den) / _2n9) / den;
function _splitEndoScalar(k, basis, n) {
  aInRange2("scalar", k, _0n13, n);
  ...
  const k2neg = k2 < _0n13;
  if (k1neg)
    k1 = -k1;
  if (k2neg)
    k2 = -k2;
  const MAX_NUM = bitMask2(Math.ceil(bitLen2(n) / 2)) + _1n13;
  if (k1 < _0n13 || k1 >= MAX_NUM || k2 < _0n13 || k2 >= MAX_NUM) {
...
function weierstrass2(params, extraOpts = {}) {
  ...
  const { endo: endoOpts, allowInfinityPoint, clearCofactor, isTorsionFree, fromBytes: fromBytes2, toBytes: toBytes3 } = extraOpts;
  const randomBytes4 = extraOpts.randomBytes === undefined ? randomBytes3 : extraOpts.randomBytes;
  if (endoOpts) {
  ...
  function pointToBytes2(_c, point, isCompressed) {
    ...
    if (isCompressed) {
      ...
      return concatBytes6(pprefix(hasEvenY), bx);
    } else {
      return concatBytes6(Uint8Array.of(4), bx, Fp2.toBytes(y));
    }
  }
  function pointFromBytes(bytes) {
    abytes4(bytes, undefined, "Point");
  ...
  const encodePoint = toBytes3 === undefined ? pointToBytes2 : toBytes3;
  const decodePoint = fromBytes2 === undefined ? pointFromBytes : fromBytes2;
  ...
  if (!isValidXY(CURVE.Gx, CURVE.Gy))
    throw new Error("bad curve params: generator point");
  const _4a3 = Fp2.mul(Fp2.pow(CURVE.a, _3n4), _4n6);
  ...
  class Point2 {
    ...
    static fromBytes(bytes) {
      const P = Point2.fromAffine(decodePoint(abytes4(bytes, undefined, "point")));
      P.assertValidity();
      return P;
    }
    static fromHex(hex) {
      return Point2.fromBytes(hexToBytes5(hex));
    }
    get x() {
      return this.toAffine().x;
    }
...
function ecdh(Point2, ecdhOpts = {}) {
  validatePointCons(Point2);
  const { Fn } = Point2;
  const randomBytes_ = ecdhOpts.randomBytes === undefined ? randomBytes3 : ecdhOpts.randomBytes;
  ...
  function randomSecretKey(seed) {
    ...
    return mapHashToField2(abytes4(seed, lengths.seed, "seed"), Fn.ORDER);
  }
  ...
  function isProbPub(item) {
    ...
    if (!isBytes5(item))
      return;
    const l = abytes4(item, undefined, "key").length;
...
function ecdsa(Point2, hash, ecdsaOpts = {}) {
  ...
  const randomBytes4 = opts.randomBytes === undefined ? randomBytes3 : opts.randomBytes;
  const hmac3 = opts.hmac === undefined ? (key, msg) => hmac2(hash_, key, msg) : opts.hmac;
  ...
  const { keygen, getPublicKey, getSharedSecret, utils: utils2, lengths } = ecdh(Point2, opts);
  ...
  const hasLargeRecoveryLifts = CURVE_ORDER * _2n9 + _1n13 < Fp2.ORDER;
  function isBiggerThanHalfOrder(number) {
  ...
  function validateSigLength(bytes, format) {
    ...
    const sizer = format === "compact" ? size3 : format === "recovered" ? size3 + 1 : undefined;
  ...
  class Signature {
    ...
    static fromBytes(bytes, format = defaultSigOpts.format) {
      ...
      if (format === "der") {
        ...
        const { r: r2, s: s2 } = DER2.toSig(abytes4(bytes), Fn.BYTES + 1);
    ...
    recoverPublicKey(messageHash) {
      ...
      const R = Point2.fromBytes(concatBytes6(pprefix((recovery & 1) === 0), x));
      const ir = Fn.inv(radj);
      const h = bits2int_modN(abytes4(messageHash, undefined, "msgHash"));
    ...
    toBytes(format = defaultSigOpts.format) {
      ...
      if (format === "recovered") {
        assertRecoverableCurve();
        return concatBytes6(Uint8Array.of(this.assertRecovery()), rb, sb);
      }
      return concatBytes6(rb, sb);
    }
    toHex(format) {
  ...
  function int2octets(num2) {
    aInRange2("num < 2^" + fnBits, num2, _0n13, ORDER_MASK);
    return Fn.toBytes(num2);
  }
  function validateMsgAndHash(message, prehash) {
    abytes4(message, undefined, "message");
    return prehash ? abytes4(hash_(message), undefined, "prehashed message") : message;
  }
  function prepSig(message, secretKey, opts2) {
    ...
    if (extraEntropy != null && extraEntropy !== false) {
      ...
      seedArgs.push(abytes4(e2, undefined, "extraEntropy"));
    }
  ...
  function verify(signature, message, publicKey, opts2 = {}) {
    ...
    publicKey = abytes4(publicKey, undefined, "publicKey");
    message = validateMsgAndHash(message, prehash);
  ...
  function recoverPublicKey3(signature, message, opts2 = {}) {
...
function sqrtMod2(y) {
  ...
  const b11 = pow22(b9, _2n10, P) * b2 % P;
  ...
  const root = pow22(t2, _2n10, P);
  if (!Fpk12.eql(Fpk12.sqr(root), y))
    throw new Error("Cannot find square root");
  return root;
}
var Fpk12 = /* @__PURE__ */ Field2(secp256k1_CURVE.p, { sqrt: sqrtMod2 });
var Pointk1 = /* @__PURE__ */ weierstrass2(secp256k1_CURVE, {
  Fp: Fpk12,
  endo: secp256k1_ENDO
});
var secp256k12 = /* @__PURE__ */ ecdsa(Pointk1, sha2563);
var TAGGED_HASH_PREFIXES2 = Object.create(null);
var enc = new TextEncoder;
var hex = (b) => "0x" + Array.from(b).map((x) => x.toString(16).padStart(2, "0")).join("");
var fromHex2 = (h) => {
  const s = h.replace(/^0x/, "");
  const out = new Uint8Array(s.length / 2);
  for (let i2 = 0;i2 < out.length; i2++)
    out[i2] = parseInt(s.slice(i2 * 2, i2 * 2 + 2), 16);
  return out;
};
var pad32 = (b) => {
  const o2 = new Uint8Array(32);
  o2.set(b, 32 - b.length);
  return o2;
};
var uint = (n) => {
  let h = n.toString(16);
  if (h.length % 2)
    h = "0" + h;
  return pad32(fromHex2(h || "00"));
};
var addr = (a) => pad32(fromHex2(a));
var cat = (...parts) => {
  const o2 = new Uint8Array(parts.reduce((s, p) => s + p.length, 0));
  let i2 = 0;
  for (const p of parts) {
    o2.set(p, i2);
    i2 += p.length;
...
var kk = (b) => keccak_2562(b);
var kstr = (s) => kk(enc.encode(s));
var DOMAIN_TYPEHASH = kstr("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)");
var VERDICT_TYPEHASH = kstr("RiskVerdict(address vault,uint64 nonce,uint64 issuedAt,uint64 expiry,uint8 reasonCode,uint64 realizedLossUsdc,bytes32 evidenceHash)");
function verdictDigest(chainId, verifyingContract, v) {
  const domain = kk(cat(DOMAIN_TYPEHASH, kstr("ShieldVault"), kstr("1"), uint(chainId), addr(verifyingContract)));
  const structHash = kk(cat(VERDICT_TYPEHASH, addr(v.vault), uint(v.nonce), uint(v.issuedAt), uint(v.expiry), uint(BigInt(v.reasonCode)), uint(v.realizedLossUsdc), fromHex2(v.evidenceHash)));
  return kk(cat(Uint8Array.from([25, 1]), domain, structHash));
}
function signEvmVerdict(chainId, verifyingContract, v, privateKeyHex) {
  const digest = verdictDigest(chainId, verifyingContract, v);
  const sig = secp256k12.sign(digest, fromHex2(privateKeyHex), { prehash: false, lowS: true, format: "recovered" });
  const rs = sig[0] < 4 ? sig.slice(1) : sig.slice(0, 64);
  const rec = sig[0] < 4 ? sig[0] : sig[64];
  return hex(cat(rs, Uint8Array.from([27 + (rec & 1)])));
}
function evmAddressOf(privateKeyHex) {
  const pub = secp256k12.getPublicKey(fromHex2(privateKeyHex), false).slice(1);
  return "0x" + hex(kk(pub)).slice(-40);
}
function evmVerdictToJson(v, signature, verifier) {
...
function base64Decode2(s) {
  const clean3 = s.replace(/[^A-Za-z0-9+/]/g, "");
  const out = [];
  let bits = 0, acc = 0;
...
function base64Encode(bytes) {
  ...
  for (let i2 = 0;i2 < bytes.length; i2 += 3) {
    const a = bytes[i2], b = bytes[i2 + 1], c = bytes[i2 + 2];
    const triple = a << 16 | (b ?? 0) << 8 | (c ?? 0);
    out += B64[triple >> 18 & 63] + B64[triple >> 12 & 63] + (b === undefined ? "=" : B64[triple >> 6 & 63]) + (c === undefined ? "=" : B64[triple & 63]);
  }
  return out;
}
function evaluateVault(io, config, vault) {
  if (!vault)
    throw new Error("trigger payload must include { vault: <address> }");
  const isEvm = config.chain === "evm";
  const secret = io.getSecret(config.secretId);
  const seed = isEvm ? new Uint8Array(0) : base64Decode2(secret);
  if (!isEvm && seed.length !== 32)
    throw new Error("verifier seed must be 32 bytes");
  if (isEvm && !/^0x[0-9a-fA-F]{64}$/.test(secret.trim()))
    throw new Error("EVM verifier secret must be a 0x-prefixed 32-byte private key");
  const viewRes = io.get(`${config.shieldApiUrl}/api/vault/${vault}`);
  if (viewRes.status < 200 || viewRes.status >= 300)
    throw new Error(`vault view request failed: ${viewRes.status}`);
  const view = JSON.parse(viewRes.body);
  const flowsRes = io.get(`${config.shieldApiUrl}/api/vault/${vault}/flows`);
  if (flowsRes.status < 200 || flowsRes.status >= 300)
    throw new Error(`flows request failed: ${flowsRes.status}`);
  const flows = JSON.parse(flowsRes.body).flows.map((f) => ({ ...f, amount: BigInt(f.amount) }));
  const now = io.now();
  ...
  io.log(`Enclave evaluation: vault=${vault} flows=${flows.length} realisedLoss24h=${a.realizedLossUsdc} trigger=${policy.lossTriggerUsdc} ` + `lossStreak=${profile.lossStreak} reloadsAfterLoss=${profile.reloadsAfterLoss7d} triggered=${a.triggered} actionable=${a.actionable}`);
  const summary = {
    vault,
    ...
    headline: a.headline,
    evidenceHash: toHex2(a.evidenceHash),
    nonce: "",
    verifier: "",
    relayed: false,
    signature: "",
  ...
  if (isEvm) {
    ...
    const ev = { vault, nonce: policy.lastVerdictNonce + 1n, issuedAt: BigInt(now), expiry: BigInt(now + 15 * 60), reasonCode: a.reasonCode, realizedLossUsdc: a.realizedLossUsdc, evidenceHash: `0x${toHex2(a.evidenceHash)}` };
    const signature2 = signEvmVerdict(BigInt(config.chainId ?? 0), config.programId, ev, key);
    verdictJson = evmVerdictToJson(ev, signature2, verifierAddr);
  } else {
    ...
    verdictJson = verdictToJson(verdict, verifier);
  }
  ...
  if (config.deliver) {
    ...
    try {
      ...
      error2 = parsed.error ?? (res.status < 300 ? "" : `relayer responded ${res.status}`);
    } catch {
      error2 = `relayer responded ${res.status}`;
    }
    io.log(`Verdict #${verdictJson.nonce} ${relayed ? `relayed on-chain: ${signature}` : `not relayed: ${error2}`}`);
  }
  return { ...summary, nonce: verdictJson.nonce, verifier: verdictJson.verifier, evidenceHash: verdictJson.evidenceHash, relayed, signature, error: error2 };
}
var configSchema = exports_external.object({
  shieldApiUrl: exports_external.string(),
  programId: exports_external.string(),
  secretId: exports_external.string(),
  deliver: exports_external.boolean(),
  chain: exports_external.enum(["solana", "evm"]).optional(),
  chainId: exports_external.number().optional()
});
function enclaveIo(runtime2) {
  const http2 = new cre.capabilities.HTTPClient;
  return {
    getSecret: (id) => runtime2.getSecret({ id }).result().value,
    get: (url) => {
      const res = http2.sendRequest(runtime2, { url, method: "GET" }).result();
    ...
    now: () => Math.floor(runtime2.now().getTime() / 1000),
    log: (msg) => runtime2.log(msg)
  };
}
var onEvaluate = (runtime2, payload) => {
  const input = JSON.parse(new TextDecoder().decode(payload.input));
  return evaluateVault(enclaveIo(runtime2), runtime2.config, input.vault ?? "");
};
var initWorkflow = () => {
