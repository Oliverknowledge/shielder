...
Substreams  https://substreams.dev/packages/shield-evm-behavioral-memory/v0.1.0
            https://substreams.dev/packages/shield-evm-behavioral-memory-sepolia/v0.1.0
            https://substreams.dev/packages/shield-behavioral-memory/v0.2.0
...
Privy (Best Financial Flow). The embedded wallet is the vault authority, not a login: ShieldVault._own() reverts for every other address. Created on login with createOnLogin: "users-without-wallets" (app/src/lib/privy.tsx:67); the app signs through its EIP-1193 provider. Wallet 0x83144b99D89947703714Ee9aA3A3614985041D2B, nonce 4 on chain. The financial flow is a $5.00 release from the vault into a Hyperliquid perps account in one transaction: 0x94960d1f937a3e36e1b16e69922a2579e77b584ed25b2ced044da7991fe889be (block 63581864) — USDC approval and transfer from the vault to CoreDepositWallet, on to the HyperCore system address 0x2000...0000, a $5.00 HyperCore credit to the registered account, and TopUpExecuted(amount=$5, instant=true, balanceAfter=$45). Supporting transactions from the same wallet: registerOwner 0x80c2a48aeeb2825fc427c2eb6b90821bc5c75fe8db38eddb3db745f55426f8f0 (block 63581683) and proposeLoosen 0xeeea692a9a7c25ada3918ceff2992c3465fff356b560b6c80a5e191cf2224b85 (block 63581938), the second of which the contract holds for 24 hours. Privy policies, quorums, session signers and Cards are not used and are not claimed. **One thing stated rather than left to be found: that release is on v1, not on the v3 contract this submission leads with.** The same embedded wallet does have a v3 vault, but it was initialised with an $8,050 protected floor against a $0 balance, and lowering a floor is a loosening, so it waits 24 hours — the contract enforcing its own asymmetry against us, which is the behaviour we are claiming. Testnet USDC is exhausted ($21.50 exists, $20 of it inside that wallet), so we could have spent it on a v3 deposit that the floor would then have locked forever. We chose not to: a deposit nothing can come back out of is not a financial flow, and the v1 transaction is one — a Privy embedded wallet moving $5.00 out of a vault, through Circle's CoreDepositWallet, into a Hyperliquid perps account, in a single transaction that the wallet signed itself. Where Privy is load-bearing on v3 is the shape of the product rather than a transaction hash: the wallet is never asked for until the user has been shown their own reload history, the replay of their worst session, and the plan Shield proposes from it. Sign-in is the last step of that funnel, not the first — open the live demo and you will not be asked for a wallet until you have pressed "Protect me from this".
