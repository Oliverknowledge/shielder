...
export function GetMeSafe({ open, onClose, context }: { open: boolean; onClose: () => void; context?: "home" | "blocked" }) {
  const { vault, balance, now, signer, actions } = useShield();
    ...
    <Sheet open={open} onClose={onClose} title={done ? undefined : "Get me safe"}>
      {done ? (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="stack" style={{ textAlign: "center", padding: "8px 0 4px" }}>
          ...
          <p className="dim">No new capital can be released until then. Everything already protected stays protected, and you can still trade what is already in {venue.label}.</p>
        ...
        <div className="stack">
          ...
          <div className="notice-list">
            <div className="notice"><span className="dot dot-protect" /><span><b>Lock new funding</b> until {clockTime(until, now)}. Every release is refused until then, whatever tonight-you asks for.</span></div>
            ...
            {bankroll > 0n && <div className="notice"><span className="dot dot-bankroll" /><span><b>Leave {usd(bankroll)} where it is.</b> Your {venue.label} account is yours; Shield never touches it.</span></div>}
          </div>
          {halved > 0n && (
