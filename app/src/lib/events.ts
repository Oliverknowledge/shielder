  ...
  switch (e.name) {
    ...
    case "PolicyTightened":
    {
      ...
      if (prev) {
        ...
        if (prev.topUpThresholdBps !== undefined && prev.topUpThresholdBps !== n("topUpThresholdBps")) parts.push(`large top-up pause from ${n("topUpThresholdBps") / 100}%`);
        ...
        if (n("cooldownUntil") > (prev.cooldownUntil ?? 0) && n("cooldownUntil") > e.blockTime) parts.push(`top-ups paused until ${clockTime(n("cooldownUntil"), now)}`);
      }
    ...
    case "TopUpExecuted":
      return { title: `Sent ${usd(s("amount"))} to ${labelOf(s("destinationOwner"))}`, body: `${d.instant ? "Instant" : "After the 30-minute pause"} · ${usd(s("velocityAfter"))} of today's limit used`, tone: "bankroll", category: "Top-up", amount: { text: `−${usd(s("amount"))}`, tone: "neutral" } };
    case "TopUpProposed":
      return { title: `Large top-up ${usd(s("amount"))} scheduled`, body: `Can move ${clockTime(n("executeAfter"), now)}`, tone: "pending", category: "Scheduled" };
    case "ColdTransferExecuted":
      return { title: `Moved ${usd(s("amount"))} to ${labelOf(s("destinationOwner"))}`, body: "Instant, within your cold-wallet cap", tone: "protect", category: "Cold wallet", amount: { text: `−${usd(s("amount"))}`, tone: "neutral" } };
    ...
    case "RiskVerdictApplied":
      return {
        title: "Loss rule paused top-ups",
