...
**Live on HyperEVM testnet, chain id 998.** `ShieldVault.sol` **v2** is deployed at
`0xba1Bb356e546AD2d036f4cAA8D25fbba4F5C1006` in tx
`0xab4e5d6af3b6b74649fd522a8255fc8606ac60d1072b3c34ced5896ea53db6a8`
(block 63626253). It is immutable — no owner, no proxy, no upgrade path.

```bash
cast call 0xba1Bb356e546AD2d036f4cAA8D25fbba4F5C1006 "VERSION()(uint8)" --rpc-url https://rpc.hyperliquid-testnet.xyz/evm   # 2
cast call 0x2B3370eE501B4a559b57D449569354196457D8Ab "balanceOf(address)(uint256)" \
  0xba1Bb356e546AD2d036f4cAA8D25fbba4F5C1006 --rpc-url https://rpc.hyperliquid-testnet.xyz/evm
```

Two vaults exist on it today, holding $70 and $6. v1 (`0xcdB6d631…`, deployed
2026-09-06) is still on chain with its four vaults; our own gauntlet found three
defects in it, all fixed in v2 and pinned as regression tests (see "Known gaps"
in `docs/THREAT_MODEL.md`). `deposit` is the only path that credits a vault, so
...
- `forge test` → `49 tests passed, 0 failed, 0 skipped (49 total tests)` —
  41 invariants in `ShieldVault.t.sol`, 6 regressions in `FixedDefects.t.sol`
  for the three v1 defects (each test's comment records what v1 did), and 2 in
  `Isolation.t.sol` for reentrancy and per-vault registry isolation.
...
The demo vault, authority `0x9872f09D96bcA7f878CEe9c4bDc8bCcA269dB006`: $70
protected, $50 floor, $10 per 24h, loss rule $3 → 12h. The CRE demo vault
(`0x751D1e26…`) is presently in a loss cooldown from a verdict signed inside
the CRE simulator after a real $7.12 loss on Hyperliquid testnet. Acting on a vault needs its authority's
...
| Claim | Status |
|---|---|
| `ShieldVault.sol` enforces the rules in `docs/THREAT_MODEL.md`; no owner, admin or upgrade path | **Verified.** 49 Foundry tests (41 invariants, 6 regressions for the three v1 defects fixed in v2, 2 isolation); deployed and immutable on chain 998; `usdc()`, `coreDeposit()` and the EIP-712 domain separator read back correctly |
...
```bash
cd contracts && forge test && cd ..    # 49: 41 invariants + 6 fixed-defect regressions + 2 isolation
...
```
contracts/               ShieldVault.sol (v2), mocks, 49 Foundry tests
                         (ShieldVault.t.sol, FixedDefects.t.sol, Isolation.t.sol)
