...
| **Chainlink CRE**: the confidential handler is load-bearing, and its verdict changes chain state | **Verified.** `handlerInTee` from `@chainlink/cre-sdk` registered at `cre/shield-risk/main.ts` with `[{ tee: "nitro", regions: ["us-west-2"] }]`. Two secrets are read in-enclave via `runtime.getSecret`: the verifier key, and — the one that makes this confidential rather than merely encrypted — **the user's private risk ladder** (`LADDER_<vault>`, mapped in `cre/secrets.yaml`), the trailing drawdown at which their reload budget shrinks. The chain holds only its salted hash; the enclave emits `{tier, ladderHash, nonce, expiry}` and no dollar figure. On-chain result on v3: tx `0x089727…b5e2ca`, `RiskTierChanged(tier=1, byVerifier=true)`, after which `effectiveVelocityThreshold()` falls from 4000000 to 3000000. Negative controls: breaking the verifier secret kills the run before any HTTP call; withholding the ladder secret stops the CLI outright (`docs/evidence/cre-simulate.txt` runs 3–4) |
...
| The package consumes live data from a Graph provider | **Verified and reproduced.** Full stateful pipeline (`map_vault_flows`) run against **two** Graph Market providers, `sepolia.eth.streamingfast.io:443` and `sepolia.substreams.pinax.network:443`, each returning the same three classified rows, plus the product-chain run against `hyperevm.substreams.pinax.network:443`. Transcript: `docs/evidence/substreams-live.txt` §1–3 |
| …and it returns rows | **Yes, from a Sepolia twin.** The Graph indexes HyperEVM **mainnet (999)** only — no testnet entry in its networks registry — so the same `ShieldVault` bytecode is deployed on Ethereum Sepolia (`0xf1ef03Ea…4f6A`, block 11654048) with a real staged history, and the package streams it back classified: DEPOSIT $10,000, TOP_UP_INSTANT $1,500 to the registered trading wallet, DEPOSIT $80 returning from it (`docs/evidence/substreams-live.txt` §1–2) |
| …and the server actually consumes those rows | **Verified** (`docs/evidence/substreams-live.txt` §6). Pointed at the twin, `/api/health` reports `source.mode: "substreams"`, connected at live head; `/api/vault/<authority>/flows` serves the flows with `source: "substreams"`; and the vault is discovered from the stream by `store_vault_registry`, with no RPC log scan on that chain. Reproduce: `SHIELD_PORT=8790 EVM_CHAIN_ID=11155111 EVM_RPC_URL=https://ethereum-sepolia-rpc.publicnode.com SHIELD_VAULT_ADDRESS=0xf1ef03Ea258EF652939bAC0250d1CDe9B5EF4f6A USDC_ADDRESS=0xb0Cbbeb2783E3965036Db1740911be45ad389837 EVM_START_BLOCK=11654048 bun run server/evm-index.ts` |
| …on the product chain (HyperEVM testnet) it returns rows | **No, and deliberately not hidden.** That package streams, authenticates and completes with zero rows (§3), because the product vault is on 998. There Shield reports `source.mode: "rpc"` and indexes logs itself, and `/api/health` says why |
